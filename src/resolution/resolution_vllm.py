...
if __name__ == "__main__":
    ...
    os.makedirs(args.out_dir + f"/resolution/{model_name}/{args.mode}/{now}", exist_ok=True)
    # set up logging to file
    logging.basicConfig(level=logging.DEBUG,
                        datefmt="%m/%d/%Y %H:%M:%S",
                        format="[%(asctime)s] {%(filename)s:%(lineno)d} %(levelname)s - %(message)s",
                        filename=args.out_dir + f"/resolution/{model_name}/{args.mode}/{now}/output.log",
                        filemode='w')
    ...
    for id_batch, batch in enumerate(tqdm(batches)):
        ...
        for id_out, out in enumerate(outputs):
            ...
            for completion in completions:
                ...
                with open(args.out_dir + f"/resolution/{model_name}/{args.mode}/{now}/completions_{args.mode}.jsonl", 'a') as f:
